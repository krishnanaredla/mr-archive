...
from modelregistry.utils.exception import *
...
class TensorInfo(object):
    ...
    def __repr__(self) -> str:
        return "Tensor({type}, {shape})".format(
            type=repr(self.dtype.name), shape=repr(self.shape)
        )
    ...
    def __repr__(self) -> str:
        if self.name is None:
            ...
            return "{name}: {info}".format(
                name=repr(self.name), info=repr(self._tensorInfo)
            )
...
class Schema(object):
    ...
    def columns(self) -> List[ColSpec]:
        ...
        if self.is_tensor_spec():
            raise SquirrelException("Not supported by TensorSpec, use `inputs` instead")
        return self._inputs
    ...
    def as_spark_schema(self):
        ...
        """
        if self.is_tensor_spec():
            raise SquirrelException("TensorSpec cannot be converted to spark dataframe")
